...
export const dataHubApi = {
  weatherForecast: (lat: number, lng: number) => apiClient.get('/api/weather/forecast', { params: { lat, lng } }),
  weatherArchive: (lat: number, lng: number) => apiClient.get('/api/weather/archive', { params: { lat, lng } }),
