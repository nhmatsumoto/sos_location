...
import { 
  ...
  AlertTriangle, Play, Camera, Activity
} from 'lucide-react';
import type { 
...
interface MapToolsSidebarProps {
  sidebarTab: 'news' | 'hotspots' | 'support' | 'volunteers';
  hotspots: Hotspot[];
  ...
  return (
    <MapToolsPanel title="Painel de ferramentas">
      ...
      <div className={`px-4 py-3 border-b border-slate-700 bg-slate-800/50 transition-all duration-300 ${sidebarTab === 'support' ? 'opacity-100 translate-y-0' : 'hidden opacity-0 -translate-y-1'}`}>
        ...
        <p className="text-[11px] text-slate-400 mb-2">Exibição exclusiva de pontos de apoio e rotas sugeridas.</p>
        <ul className="space-y-2 max-h-48 overflow-y-auto pr-1">
          {supportPoints.map((point) => (
            <li key={point.id} className="text-xs bg-slate-900/60 border border-slate-700 rounded-md p-2">
              ...
              {point.notes && <p className="text-slate-500 italic mt-1">{point.notes}</p>}
            </li>
          ))}
        </ul>
      </div>
